...
%% CUBIC SEARCH
function []= HW2_Cubicsearch_x_AbhayKela
fprintf('Solution by Cubic search in terms of X for different convergence parameters\n\n');
syms x;
...
xm=vpasolve(dfds);
xm=double(xm(real(xm)>=x1&real(xm)<=x2&imag(xm)==0));
xa=abs((xm-xopt)/xopt);
e=0.1;
i=1;
while xa>=e;
